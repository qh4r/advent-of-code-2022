...
const executeMove = ({elv, newLocation}) => {
    ...
    // console.log('neww', newLocation, elv)
...
console.log('end', field.map(row => row.join('')).join('\n'));
console.log(extremes)
let empty = 0
for (let y = extremes.minY; y <= extremes.maxY; y++) {
    for (let x = extremes.minX; x <= extremes.maxX; x++) {
        if(field[y][x] === '.') empty += 1;
    }
}
