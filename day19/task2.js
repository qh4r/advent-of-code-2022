...
const makeZeroMap = () => new Map(Array.from({length: 32}).map((_, i) => [i, 0]));

const bestPlans = lines.slice(0,3).map((line,li) => {
    ...
    if (match) {
        ...
        const segregatePlans = (plan) => {
            const timeLeft = (32 - plan.steps.length);
...
console.log("task2 -> ", bestPlans.reduce((sum, plan) => {
    return sum * plan.resources.geode;
}, 1));
